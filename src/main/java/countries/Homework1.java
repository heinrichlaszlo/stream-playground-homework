...
public class Homework1 {
    ...
    public void streamPipeline5() {
        countries.stream().sorted(Comparator.comparingLong(country -> country.getPopulation())).map(country -> country.getName()).limit(10).forEach(System.out::println);
    }
    ...
    public void streamPipeline7() {
        countries.stream().sorted(Comparator.comparing(country -> country.getTimezones().size())).map(country -> country.getName()).forEach(System.out::println);
    }
...
ed by the language code "es").
    ...
    public void streamPipeline15() {
        countries.stream().filter(country -> country.getArea() != null && country.getArea().intValue()<1).map(country -> country.getName()).forEach(System.out::println);
    }
    ...
    public void streamPipeline16() {
        countries.stream().filter(country -> (country.getRegion().equals(Region.ASIA) || (country.getRegion().equals(Region.EUROPE)))).flatMap(country -> country.getTimezones().stream()).distinct().forEach(System.out::println);
    }
